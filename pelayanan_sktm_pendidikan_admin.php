...
// Tentukan tabel berdasarkan jenis SKTM
$jenis = isset($_GET['jenis']) ? $_GET['jenis'] : 'Pendidikan';
$tableName = $jenis;
...
// Query dasar
$baseQuery = "SELECT ID, nama, pengurusan, alamat, tanggal, penanggung_jawab FROM `$tableName` WHERE 1=1";
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $baseQuery .= " AND (nama LIKE '%$search%' OR pengurusan LIKE '%$search%' OR alamat LIKE '%$search%')";
}

// Filter kategori
/*$kategori = [];
if ($jenis == 'Kesehatan') {
    $kategori = ['BPJS', 'Jampersal', 'Jamkesda', 'KIS'];
} elseif ($jenis == 'Pendidikan') {
    $kategori = ['KIP', 'Beasiswa', 'Biaya Kuliah'];
} elseif ($jenis == 'Umum') {
    $kategori = ['Lain-lain'];
}
...
if (!empty($kategori)) {
    ...
    $baseQuery .= " AND pengurusan IN ($kategoriStr)";
}*/
...
<head>
    ...
    <title>Pelayanan SKTM | Admin</title>
    ...
    <style>
        ...
        .table td:nth-child(3) { /* Kolom keempat (Nama) */
            text-align: left;
        }

        .table td:nth-child(4) { /* Kolom keempat (Alamat) */
            text-align: left;
        }
...
<div class="content">
    ...
    <h1 style="text-align: left; font-weight:800; margin-top: 40px; margin-bottom: 40px;">PELAYANAN SKTM - PENDIDIKAN</h1>
        ...
        <tr>
                ...
                <th style="width: 15%;">Pengurusan</th>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>
                              <div class='btn-group btn-group-sm' role='group'>
                                <button class='btn btn-warning edit-data' data-id='" . $row['ID'] . "' data-nama='" . htmlspecialchars($row['nama']) . "' data-pengurusan='" . htmlspecialchars($row['pengurusan']) . "' data-alamat='" . htmlspecialchars($row['alamat']) . "' data-tanggal='" . $row['tanggal'] . "' data-penanggung_jawab='" . htmlspecialchars($row['penanggung_jawab']) . "' title='Edit'><i class='fas fa-edit'></i></button>
                                <button class='btn btn-danger hapus-data' data-id='" . $row['ID'] . "' title='Hapus'><i class='fas fa-trash'></i></button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="formTambahData">
          ...
          <div class="form-group">
            <label for="pengurusan">Pengurusan</label>
            <input type="text" class="form-control" id="pengurusan" name="pengurusan" required>
          </div>
          <div class="form-group">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
            <label for="editPengurusan">Pengurusan</label>
            <input type="text" class="form-control" id="editPengurusan" name="pengurusan" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="editPenanggungJawab" name="penanggungJawab" required>
        </div>
        </form>
      </div>
      <div class="modal-footer">
...
$(document).ready(function() {
  $('#btnSimpan').click(function() {
    ...
    $.ajax({
      url: 'tambah_data_pendidikan.php',
      type: 'POST',
      data: formData,
  ...
  $('#penanggungJawab').on('input', function() {
...
$(document).ready(function() {$(document).ready(function() {
    ...
    var pengurusan = $(this).data('pengurusan');
    var alamat = $(this).data('alamat');
    var tanggal = $(this).data('tanggal');
    var penanggungJawab = $(this).data('penanggung_jawab');
    
    $('#editId').val(id);
    $('#editNama').val(nama);
    $('#editPengurusan').val(pengurusan);
    $('#editAlamat').val(alamat);
    $('#editTanggal').val(tanggal);
    $('#editPenanggungJawab').val(penanggungJawab);
    ...
    $('#btnSimpanEdit').click(function() {
        ...
        $.ajax({
            url: 'update_data_pendidikan.php',
            type: 'POST',
            data: formData,
            success: function(response) {
    ...
    $('#editPenanggungJawab').on('input', function() {
        this.value = this.value.toUpperCase();
        });
    });
});
...
$(document).ready(function() {
    ...
    $.ajax({
        url: 'upload_dokumen_pendidikan.php',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
    ...
    $('.download-berkas').click(function() {
        var id = $(this).data('id');
        window.location.href = 'download_berkas_pendidikan.php?id=' + id;
    });
});
