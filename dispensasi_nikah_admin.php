...
$jenis = isset($_GET['jenis']) ? $_GET['jenis'] : 'Dispensasi_nikah';
$tableName = $jenis;
...
// Query dasar
$baseQuery = "SELECT ID, nama_suami, nama_istri, nomor_surat, pengantar_kua, tanggal FROM `$tableName` WHERE 1=1";

// Filter berdasarkan tahun jika ada
if (isset($_GET['year']) && !empty($_GET['year'])) {
    $year = $conn->real_escape_string($_GET['year']);
    $baseQuery .= " AND YEAR(STR_TO_DATE(tanggal, '%d-%m-%Y')) = '$year'";
}

// Pencarian
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search = $conn->real_escape_string($_GET['search']);
    $baseQuery .= " AND (nama_suami LIKE '%$search%' OR nama_istri LIKE '%$search%' OR nomor_surat LIKE '%$search%' OR tanggal LIKE '%$search%')";
}
...
<head>
    ...
    <title>Dispensasi Nikah | Admin</title>
...
<div class="content">
    ...
    <h1 style="text-align: left; font-weight:800; margin-top: 40px; margin-bottom: 40px;">Daftar Permohonan Dispensasi Nikah</h1>
        ...
        <thead>
            <tr>
                ...
                <th style="width: 25%;">Pengantar KUA</th>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>
                                <div class='btn-group btn-group-sm' role='group'>
                                    <button class='btn btn-secondary btn-warning edit-data mr-1' data-id='" . $row['ID'] . "' data-nama_suami='" . htmlspecialchars($row['nama_suami']) . "' data-nama_istri='" . htmlspecialchars($row['nama_istri']) . "' data-nomor_surat='" . htmlspecialchars($row['nomor_surat']) . "' data-pengantar_kua='" . htmlspecialchars($row['pengantar_kua']) . "' data-tanggal='" . $row['tanggal'] . "'title='Edit'><i class='fas fa-edit'></i></button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="formTambahData">
          ...
          <div class="form-group">
            <label for="nama">Nama Istri</label>
            <input type="text" class="form-control" id="namaIstri" name="namaIstri" required>
          </div>
          <div class="form-group">
            <label for="alamat">Nomor Surat</label>
            <textarea class="form-control" id="nomorSurat" name="nomorSurat" required></textarea>
          </div>
          <div class="form-group">
            <label for="penanggungJawab">Pengantar KUA</label>
            <input type="text" class="form-control" id="pengantarKUA" name="pengantarKUA" required>
          </div>
          <div class="form-group">
...
<div class="modal fade" id="editDataModal" tabindex="-1" role="dialog" aria-labelledby="editDataModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editDataModalLabel">Edit Data Dispensasi Nikah</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="formEditData">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="editNamaSuami" name="editNamaSuami" required>
          </div>
          <div class="form-group">
            <label for="editNamaIstri">Nama Istri</label>
            <input type="text" class="form-control" id="editNamaIstri" name="editNamaIstri" required>
          </div>
          <div class="form-group">
            <label for="editNomorSurat">Nomor Surat</label>
            <input type="text" class="form-control" id="editNomorSurat" name="editNomorSurat" required>
          </div>
          <div class="form-group">
            <label for="editPengantarKUA">Pengantar KUA</label>
            <input type="text" class="form-control" id="editPengantarKUA" name="editPengantarKUA" required>
          </div>
          <div class="form-group">
...
$(document).ready(function() {
  $('#btnSimpan').click(function() {
    ...
    $.ajax({
      url: 'tambah_data_dispensasi.php',
      type: 'POST',
      data: formData,
...
$(document).ready(function() {$(document).ready(function() {
    ...
    var id = $(this).data('id');
    var namaSuami = $(this).data('nama_suami');
    var namaIstri = $(this).data('nama_istri');
    var nomorSurat = $(this).data('nomor_surat');
    var pengantarKUA = $(this).data('pengantar_kua');
    var tanggal = $(this).data('tanggal');

    $('#editId').val(id);
    $('#editNamaSuami').val(namaSuami);
    $('#editNamaIstri').val(namaIstri);
    $('#editNomorSurat').val(nomorSurat);
    $('#editPengantarKUA').val(pengantarKUA);
    ...
    $('#btnSimpanEdit').click(function() {
        ...
        $.ajax({
            url: 'update_data_dispensasi.php',
            type: 'POST',
            data: formData,
            success: function(response) {
...
$(document).ready(function() {
    ...
    $.ajax({
        url: 'upload_dokumen_dispensasi.php',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
    ...
    $('.download-berkas').click(function() {
        var id = $(this).data('id');
        window.location.href = 'download_berkas_dispensasi.php?id=' + id;
    });
});
